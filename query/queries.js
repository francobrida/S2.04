...
db.restaurants.find({"grades.score": {$gt: 80}, "grades.score": {$lt: 100}}, {_id: 0})
...
db.restaurants.find({"location.coordinates": {$near: {$geometry:{type: "Point",coordinates: [-74, 40.7]}, $maxDistance: 5000}}},{restaurant_id:1,name:1,"address.street":1, "address.zipcode":1,"location.coordinates":1, _id:0})
...
db.restaurants.find({}, {_id: 0}).sort({cuisine:1, borough:-1, _id:0})
...
db.restaurants.find({name: /mon/}, {name:1, borough:1, "location.coordinates.0":1, "location.coordinates.1":1, cuisine:1, _id:0})

// 32. Mostrar restaurant_id, name i grade i score de més de 80 però menys que 100.
db.restaurants.find({grades: {$elemMatch:{score: { $gt:80, $lt:100}}}}, {restaurant_id:1, name:1, "grades.grade":1, "grades.score":1, _id:0})
